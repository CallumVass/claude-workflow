---
name: review-plugins
description: Domain-specific review plugin router. Scans plugins, matches triggers against a diff, and returns which plugins to load.
---
...
Each plugin lives in a subdirectory of `skills/review-plugins/`:

```
skills/review-plugins/
  <name>/
...
3. **Plugin references** — read files from `references/` only when a specific finding needs deeper context to verify. Expensive, on-demand only.
